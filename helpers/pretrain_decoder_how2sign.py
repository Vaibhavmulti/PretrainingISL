...
def train_tokenizer_from_scratch(texts, vocab_size=vocab_size_decoder, min_frequency=2):
    ...
    os.makedirs("custom_tokenizer_how2sign", exist_ok=True)
    tokenizer.save_model("custom_tokenizer_how2sign")
    
    return tokenizer


# def train_tokenizer_from_scratch(texts, vocab_size=vocab_size_decoder, min_frequency=2):
#     # Initialize a ByteLevelBPE tokenizer
#     tokenizer = ByteLevelBPETokenizer(dropout=None, unicode_normalizer="nfc")
...
#     tokenizer.train(
#         files=["temp_train.txt"],
#         vocab_size=vocab_size - 7, # Reserve space for special tokens
#         min_frequency=min_frequency,
        
#     )
#     # Add special tokens with explicit IDs
#     special_tokens_dict = {
#         "<s>": 0,
#         "<pad>": 1,
#         "</s>": 2,
...
#     os.remove("temp_train.txt")
#     def verify_special_tokens(tokenizer):
#         for token in special_tokens_dict.keys():
#             encoded = tokenizer.encode(token).ids
#             if len(encoded) != 1:
#                 print(f"Warning: {token} is not treated as a single token! IDs: {encoded}")
...
def main():
    ...
    df1 = pd.read_csv('/DATA3/vaibhav/isign/PretrainingISL/train_MTWASL16M.csv')
    #df2 = pd.read_csv('/DATA3/vaibhav/isign/PretrainingISL/isign_new.csv')
    texts = df1['text'].tolist() + train_df2['text'].tolist()
    ...
    trained_model.save_pretrained('custom_gpt2_how2sign/final_model')
    tokenizer.save_model('custom_gpt2_how2sign')
