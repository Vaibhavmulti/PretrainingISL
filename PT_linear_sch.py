...
sub_project_name = "Trial_Trial_PreTraining2"
run_name = "Trial_Trial_PT2"
...
#wandb.init(project=project_name, config = hyperparameters, id="6pub6l17", resume="must")
...
train_dataset = FeatureVectorDataset(train_video_uids,tokenizer_target,
                                      randomize_word_order, train_labels,
                                      max_frames=MAX_FRAMES, video_dir=POSE_DIR,
                                      step_frames=None, add_noise = True)
test_dataset = FeatureVectorDataset(test_video_uids,tokenizer_target, 
                                    randomize_word_order, test_labels,
                                    max_frames=MAX_FRAMES, video_dir=POSE_DIR,
                                    step_frames=None, add_noise = True)
eval_dataset = FeatureVectorDataset(eval_video_uids,tokenizer_target, 
                                    randomize_word_order,eval_labels,
                                    max_frames=MAX_FRAMES, video_dir=POSE_DIR,
                                    step_frames=None, add_noise = True)

eval2_dataset = FeatureVectorDataset_Isign(eval2_video_uids, tokenizer_target, 
                                        eval2_labels, max_frames=MAX_FRAMES, video_dir=POSE_DIR_ISIGN,
                                        step_frames=None, add_noise = True)
...
def save_checkpoint(model, feature_projection, optimizer, scheduler, epoch, best_val_B4, best_val_loss, checkpoint_path, current_step,
                    best_val_B4_isign, best_val_loss_isign, best_val_B1_isign):
    checkpoint = {
        'epoch': epoch,
...
def load_checkpoint(model, feature_projection, optimizer, scheduler, checkpoint_path):
    if os.path.exists(checkpoint_path):
        ...
        print("*"*50)
        return start_epoch, best_val_B4, best_val_loss, best_val_B4_isign, best_val_loss_isign, best_val_B1_isign
    else:
...
if os.path.exists(checkpoint_path):
    start_epoch, best_val_B4, best_val_loss, best_val_B4_isign, best_val_loss_isign, best_val_B1_isign = load_checkpoint(
...
def model_eval(eval_loader, log_what, best_val_B4,best_val_loss,best_val_B4_isign,best_val_B1_isign,best_val_loss_isign,counter, current_step, save_model=False):
    model.eval()
    ...
    if log_what == "CISLR":
        if bleu4 > best_val_B4 or (bleu4 == best_val_B4 and avg_eval_loss < best_val_loss):
            ...
            save_checkpoint(
                ...
                epoch, best_val_B4, best_val_loss, best_checkpoint_path, current_step, best_val_B4_isign, best_val_loss_isign, best_val_B1_isign
            )
        ...
        if counter >= 1:
            if bleu4 > best_val_B4_isign or (bleu4 == best_val_B4_isign and avg_eval_loss < best_val_loss_isign):
                ...
                save_checkpoint(
                    model, feature_projection, optimizer, scheduler,
                    epoch, best_val_B4, best_val_loss, best_checkpoint_path_isignB4, current_step, best_val_B4_isign, best_val_loss_isign, best_val_B1_isign
                )
            ...
            if bleu1 > best_val_B1_isign or (bleu1 == best_val_B1_isign and avg_eval_loss < best_val_loss_isign):
                ...
                save_checkpoint(
                    model, feature_projection, optimizer, scheduler,
                    epoch, best_val_B4, best_val_loss, best_checkpoint_path_isignB1, current_step, best_val_B4_isign, best_val_loss_isign,best_val_B1_isign
                )
    ...
    for batch in progress_bar:
        ...
        if epoch_steps % 2500 == 0:
            ...
            best_val_B4, best_val_loss, best_val_B4_isign, best_val_B1_isign, best_val_loss_isign = model_eval(
                ...
                best_val_B1_isign, best_val_loss_isign, counter, epoch_steps, save_model=True)
            best_val_B4, best_val_loss, best_val_B4_isign, best_val_B1_isign, best_val_loss_isign = model_eval(
